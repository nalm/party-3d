...
export const AXES = [
  ...
  {
    key: 'norms',
    // 세로축(y). 값이 커질수록 위 — 반다원주의로 "올라간다"가 직관에 맞는다.
    threeAxis: 'y',
    name: '민주주의 규범',
    color: '#eb6834',
    colorHex: 0xeb6834,
...
export const DROPLINE = {
  ...
  // 축을 바꾼 뒤 이 선의 길이가 곧 규범 점수이고 바닥의 착지점이 2D 산점도 좌표다.
...
export const CAMERA = {
  ...
  // 점끼리 겹치지 않는 방향을 실측해서 고른다. 축 배치를 바꾸면 겹침 구조도 바뀌므로
  // 그때마다 다시 재야 한다. 규범↔문화 교체 후 23개 점의 최소 화면 간격을 측정해
  // [24,18,22](0.0068, fr-lfi와 fr-rn이 거의 겹침)에서 [28,18,18](0.0307)로 옮겼다.
  initialDir: [28, 18, 18],
